...
export default function HumanVerification() {
  ...
  useEffect(() => {
    try {
      ...
      const userId = `human-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
      
      // Create Self app configuration for human verification
      const app = new SelfAppBuilder({
        version: 2,
        appName: "Human Verification",
        scope: "human-verification-check",
        endpoint: "http://localhost:3002/api/verify-human",
        ...
        disclosures: {
          // Simple human verification - just age check
          minimumAge: 18,  // Must be 18+ to be considered human adult
          // Optional: Add nationality check to prevent bots from certain regions
          nationality: true,
          // Security check
          ofac: true
        }
      ...
      setVerificationError('Failed to initialize human verification system');
      setIsLoading(false);
    }
  }, []);
    ...
    return (
      <div className="verification-container">
        <div className="loading">
          <h2>🤖 Initializing Human Verification...</h2>
          <p>Please wait while we set up the verification system.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="verification-container">
      <div className="verification-header">
        ...
        <p className="verification-description">
          Prove you are human by verifying your identity with a government-issued document. 
